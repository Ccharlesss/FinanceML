...
# Load the .env file with all settings
env_path = Path(".") / ".env"
load_dotenv(dotenv_path=env_path)

class Settings(BaseSettings):
    """Configuration variables for the FastAPI application."""
    ...
    @property
    def DATABASE_URL(self) -> str:
        """Construct the database URL from the configuration variables."""
        if self.TESTING:
            return "sqlite:///src/Unit_test/test.db"  # Use SQLite for testing in the specified directory
