...
async def get_prediction(choosen_ticker: str, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
  if not current_user:
    raise HTTPException(status_code=401, detail="Not authorized.")
  if not current_user.role == "Admin":
    raise HTTPException(status_code=401, detail="Not an admin thus not authorized.")
  await RandomForesstService.orchestrate_random_forest(choosen_ticker)
