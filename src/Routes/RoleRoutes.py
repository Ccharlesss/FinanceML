...
async def update_role(data: UpdateRoleFields, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    await UserController.update_user_role(data.user_id, data.new_role, session)
    return {"message": f"User with email {data.user_id} has been assigned a new role: {data.new_role}"}
